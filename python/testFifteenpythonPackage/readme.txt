testFifteen MATLAB Python Package
...
2. Installing the testFifteen Package

A. Change to the directory that contains the file setup.py and the subdirectory 
testFifteen. If you do not have write permissions, copy all its contents to a temporary 
...
In the following directions, replace MR/R2025a by the directory on the target machine where MATLAB is installed, or MR by the directory where the MATLAB Runtime is installed.
...
3. Using the testFifteen Package

The testFifteen package is on your Python path. To import it into a Python script or 
session, execute:
